...
class LaporHrController extends Controller
{
    public function index()
    {
        ...
        $counts = [
            'total' => LaporHr::count(),
            'open' => LaporHr::where('status', 'open')->count(),
            'in_progress' => LaporHr::where('status', 'on progress')->count(),
            'closed' => LaporHr::where('status', 'close')->count(),
        ];

        return view('lapor_hr.index', compact('employees', 'reportCategory', 'status', 'counts'));
    }
