...
class PresenceSummaryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Get Employee based on conditions
        $employees = Employee::with('overtimes', 'presences', 'WorkDay');

        //conditions for division_id and department_id
        if ($userDivision && !$userDepartment) {
            $employees->where('division_id', $userDivision);
        } elseif (!$userDivision && $userDepartment) {
            $employees->where('department_id', $userDepartment);
        } elseif ($userDivision && $userDepartment) {
            $employees->where('division_id', $userDivision)
                      ->where('department_id', $userDepartment);
        }

        $employees = $employees->get(); // Fetch filtered employees

        $employees->each(function ($employee) use ($startDate, $endDate) {
            // Your existing logic for processing employees' presence, overtime, leaves, etc.
            ...
            // Other logic for presence, absence, overtime, late arrivals, etc.
            $employee->presence = Presence::where('employee_id', $employee->id)
                ->whereBetween('date', [$countStartDate, $countEndDate])
                ->count('date'); // Total Presence
