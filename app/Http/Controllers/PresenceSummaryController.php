...
class PresenceSummaryController extends Controller
{
    ...
    //         }
    //         $employee->late_arrival = $late_arrival->where('late_arrival', 1)->where('status', Presence::STATUS_PRESENCE)->count();
    ...
    public function index(Request $request)
    {
        ...
        if ($userDivision && !$userDepartment) {
            $query->whereHas('position', fn($q) => $q->where('division_id', $userDivision));
        } elseif (!$userDivision && $userDepartment) {
            $query->whereHas('position', fn($q) => $q->where('department_id', $userDepartment));
        }

        $employees = $query->get();

        // Let the Trait handle all the heavy lifting (Holidays, Alpha, Effective Days)
        $employees = $this->calculatePresenceSummary($employees, $startDate, $endDate);
